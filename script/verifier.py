...
def main():
    ...
    inference_results_dir = Path(args.inference_results_dir)
    verify_results_dir = Path(args.verify_results_dir)
    ...
    if args.type == "unexpected_transition":
        ...
        if args.scenario == "webarena":
            from verifier import VerifyPopupWebarena

            verifier = VerifyPopupWebarena(logger, force_verify=args.force)
